...
#include "GSFTable.dat" // just placeholder values in this file for now
  ...
  const double g_adSigE1[] = {206.00, 0.00}; // mb magnitude
  const double g_adEneE1[] = { 16.84, 0.01}; // MeV centroid energy, non-zero
  const double g_adGamE1[] = {  4.50, 0.00}; // MeV GDR width
  //                                  ^^^^ for a 2nd resonance
  ...
  #ifdef bE2_SingPart
  const double g_dSpSigE2 = 3e-11; // MeV^-5
  #endif
